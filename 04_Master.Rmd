...
>*Answer 1: The satellite can only effectively collect data in two spatial dimensions (i.e. it can capture the greenness of the Earth's surface but not vertical growth). The measure of greenness therefore saturates over time because only the initial post-fire growth of the plants is accounted for (not their longer-term maturation). Once green plants exist in a certain area, their growth thereafter does not contribute to more greenness.*
<br>
...
>*Answer 2: The estimates of alpha and lambda have smaller magnitudes for the full model relative to the simple model (0.2075 < 0.2511 and 0.9892 < 1.1769 respectively), but the estimate of gamma has a greater magnitude (0.3647 > 0.3237).*
<br>
...
>*Answer 3: The full model (Model 2) is better. Adding A and phi as parameters substantially improves the fit of the model (as seen by the p-value of < 0.001).*
Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4: There are two degrees of freedom in this ANOVA as a result of adding two parameters (A and phi).*
<br>
...
>*Answer 5: Yes, the full model is preferred because it has a much lower AIC value relative to the simple model.*
<br>
...
>*Answer 6: In this case, we are using the same data and parameters for both models, but only the latter model (MLE) uses log-likelihood maximisation, rendering the two models incomparable. "The theory of AIC requires that the log-likelihood has been maximized: whereas AIC can be computed for models not fitted by maximum likelihood, their AIC values should not be compared."* 
<br>